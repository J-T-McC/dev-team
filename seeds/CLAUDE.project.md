...
- **Documents go forward.** Never write a PRD, plan, or ADR for work already shipped; existing architecture gets an ADR the first time someone changes it. Work adopted mid-flight is marked `pre-adoption` in `docs/status.md` — upstream gates waived, downstream gates apply.
